...
# TRAINING FUNCTION
# ----------------------------
def train_one_epoch(model, data_loader, optimizer, lr_scheduler, epoch, amp_autocast, loss_scaler=None, log_dir=None, class_weights=None, val_loader=None, teacher_model=None, unsup_weight=1.0, bgcdf_loss_fn=None):
    """
    ...
    sup_epoch = unsup_epoch = total_epoch = bgcdf_epoch = 0.0
    ...
    for batch_idx, batch in enumerate(data_loader):
        ...
        with amp_autocast():
            ...
            if teacher_model_eval is not None and unlabeled_mask.any():
                ...
                # BG-CDF LOSS (NEW)
                # ==========================
                if bgcdf_loss_fn is not None:                    
                    #bgcdf = bgcdf_loss_fn(student_probs_unl, teacher_probs)
                    
                    confidence = teacher_probs.max(dim=1)[0]  # (B,H,W)
                    valid_bgcdf = (confidence > 0.7).float()
                    
                    bgcdf = bgcdf_loss_fn(student_probs_unl, teacher_probs)
                    bgcdf = (bgcdf * valid_bgcdf).sum() / (valid_bgcdf.sum() + 1e-6)
                    
                    if batch_idx % 20 == 0:
                        print(f"BG-CDF computed: {bgcdf.item():.6f}", flush=True)
                    
                    bgcdf = torch.nan_to_num(bgcdf, nan=0.0, posinf=10.0, neginf=0.0)
                    bgcdf = torch.clamp(bgcdf, 0.0, 5.0)
                    #bgcdf_epoch += float(bgcdf.item()) * bs
                    num_unl = unlabeled_mask.sum().item()
                    bgcdf_epoch += float(bgcdf.item()) * num_unl
                    
                else:
                    bgcdf = torch.tensor(0.0, device=ptu.device)
                ...
                unsup_loss = (lambda_ce * ce_unsup + lambda_dice * dice_unsup + lambda_bgcdf * bgcdf)
                
                unsup_loss = unsup_loss / (lambda_ce + lambda_dice + lambda_bgcdf)
